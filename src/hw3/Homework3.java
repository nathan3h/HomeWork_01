...
//		的號碼與總數
	...
	private static void printLottery() {
		...
		for (int i = 1; i <= 49; i++) {
			if (i % 10 == numberToExclude)
				continue;
			if (numberToExclude == 1 && i >= 10 && i < 20)
				continue;
			if (numberToExclude == 2 && i >= 20 && i < 30)
				continue;
			if (numberToExclude == 3 && i >= 30 && i < 40)
				continue;
			if (numberToExclude == 4 && i >= 40 && i < 50)
				continue;
			System.out.print(i + "\t");
			total++;
		...
		availableNumbers = Arrays.copyOf(availableNumbers, total);
		int count = 0;
		int[] selectedNumbers = new int[6];
		while (count < 6) {
			int index = (int) (Math.random() * total);
			int[]sortedNumbers = Arrays.copyOf(selectedNumbers, selectedNumbers.length);
			Arrays.sort(sortedNumbers);
			int indexReturned = Arrays.binarySearch(sortedNumbers, availableNumbers[index]);
			if (indexReturned < 0) {
				selectedNumbers[count] = availableNumbers[index];
				System.out.print(selectedNumbers[count] + "\t");
				count++;
			}
		}
	}
